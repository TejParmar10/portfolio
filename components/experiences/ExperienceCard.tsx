...
const Experience = ({ index, company, position, desc, institute, degree, duration, GPA, percentage }: ExperienceProps) => {
  ...
  return (
    <div className={`mb-6 md:mb-8 flex md:justify-between items-center w-full ${index % 2 === 0 ? 'md:flex-row-reverse left-timeline' : 'right-timeline'}`}>
      ...
      <span className="z-20 flex items-center order-1 justify-center w-6 h-6 md:w-9 md:h-9 bg-violet-200 rounded-full ring-4 md:ring-8 ring-white dark:ring-grey-800 dark:bg-violet-900">
        {company ? <MdWork className="text-base md:text-xl text-violet-600 dark:text-violet-400" /> : <MdSchool className="text-base md:text-xl text-violet-600 dark:text-violet-400" />}
      </span>
      ...
      <motion.div
        ...
        {GPA && <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">GPA: {GPA}</p>}
        {percentage && <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Percentage: {percentage}</p>}
